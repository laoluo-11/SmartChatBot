 ...
 * 这样可以避免不同模块之间变量名互相打架。 */
 ...
 static const char *TAG = "mic";  //本模块日志标签
 static i2s_chan_handle_t mic_rx_chan=NULL;  //I2S接收通道句柄
...
void mic_init(void)
{
    ...
    i2s_std_config_t std_cfg ={
        ...
        .gpio_cfg={
            ...
            .din = MIC_DIN_GPIO,      // 数据输入 → GPIO16（接麦克风 SD，声音从这里进来）
            .invert_flags = {
    ...
    ESP_LOGI(TAG,"I2S 麦克风已启动: %d Hz, 16bit, MONO", SAMPLE_RATE);
}
 ...
 * ------------------------------------------------------------------------- */
 ...
 void mic_task(void *pvParameters)
 {
    ...
    while (1)
    {
        ...
        esp_err_t r = i2s_channel_read(mic_rx_chan,
            buf,
            MIC_READ_SAMPLES * sizeof(int16_t),
            &bytes_read,
            pdMS_TO_TICKS(1000));
        if(r == ESP_OK && bytes_read >0){  //读取到数据
            ...
            float rms = sqrtf((float)((double)sum/samples));
