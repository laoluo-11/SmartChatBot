...
#include "audio_out.h"             // 本模块声明（引脚宏、audio_out_init/play_tone/task 原型）
#include <stdio.h>                // 标准输入输出
#include <math.h>                 // 数学库，用 sinf() 生成正弦波
#include "freertos/FreeRTOS.h"    // FreeRTOS 内核
#include "freertos/task.h"        // 任务函数（vTaskDelay）
...
static int g_volume = 70;                          // 当前音量（百分比 0~100），默认 70%
...
void audio_out_init(void)
{
    ...
    ESP_ERROR_CHECK(i2s_channel_enable(spk_tx_chan));  // 5) 启用通道：正式开始"能发声"

    ESP_LOGI(TAG, "I2S 喇叭已启动: %d Hz, 16bit, MONO", SPK_SAMPLE_RATE);
}

/* -------------------------------------------------------------------------
 * audio_out_play_tone：造一段正弦波并送进功放播放
 * ------------------------------------------------------------------------- */
void audio_out_play_tone(uint16_t freq_hz, uint32_t duration_ms)
{
    if (spk_tx_chan == NULL) {   // 防御：喇叭没初始化就别播，打印错误直接返回
        ESP_LOGE(TAG, "喇叭未初始化，无法播放");
        return;
    }
    ...
    if (!buf) {   // 内存申请失败兜底
        ESP_LOGE(TAG, "音频缓冲 malloc 失败");
        return;
    }

    /* 生成正弦波：每个样本 = sin(2π·f·t) × 振幅。
     *   t = i / sr 表示第 i 个样本的时间点（秒）
     *   振幅 = 30000 × 音量百分比（30000 不是满量程 32767，留余量防破音；
     *   音量 0 时振幅为 0，相当于静音但不报错）。 */
    int amplitude = (int)(30000.0f * (float)g_volume / 100.0f);
    ...
    while (offset < (size_t)total) {
        ...
        esp_err_t r = i2s_channel_write(spk_tx_chan, buf + offset, chunk, &written, pdMS_TO_TICKS(1000));
...
/* -------------------------------------------------------------------------
 * 音量控制：四个函数都只动 g_volume 这一个变量，I2S 配置完全不用改。
 * play_tone 在生成正弦波时自动按 g_volume 缩放振幅——这就是"调音量"的本质。
 * ------------------------------------------------------------------------- */
void audio_out_set_volume(int vol)
{
    if (vol < AUDIO_VOL_MIN) vol = AUDIO_VOL_MIN;   // 低于下限就夹到下限
    if (vol > AUDIO_VOL_MAX) vol = AUDIO_VOL_MAX;   // 高于上限就夹到上限
    g_volume = vol;
...
void audio_out_volume_up(void)
{
    audio_out_set_volume(g_volume + 10);   // +10，到 100 自动封顶
}

void audio_out_volume_down(void)
{
    audio_out_set_volume(g_volume - 10);   // -10，到 0 自动到底（静音）
}
